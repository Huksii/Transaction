...
def validate_phone_number(phone_number):
    ...
    if len(phone_number) < 12:
        return ValidationError(choice(len_phone_text))
    
    if not phone_number.startswith('+'):
        return ValidationError(choice(format_phone_text))
    
    numbers = '0123456789'
    for char in phone_number[1:]:
        if char not in numbers:
            return ValidationError(choice(exception_phone_text).format(char))